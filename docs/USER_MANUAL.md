...
> **팁**: 활동이 있는 날짜는 **파란색 링크**로 표시됩니다. 클릭하면 해당 날짜의 대화 내역을 확인할 수 있습니다.

## 7.5 대화 내역 조회

관리자는 특정 날짜의 모든 대화 세션과 메시지 내용을 상세히 확인할 수 있습니다.

### 대화 내역 접속 방법

1. **통계 대시보드**에서 **일별 상세** 테이블 확인
2. 활동이 있는 날짜(세션 또는 메시지 > 0)의 **날짜 링크** 클릭
3. 해당 날짜의 대화 내역 페이지로 이동

```
통계 페이지                      대화 내역 페이지
┌────────────────┐              ┌─────────────────────────────────────┐
│ 일별 상세       │              │ 대화 내역 - 2024-12-26              │
├────────────────┤              ├───────────────┬─────────────────────┤
│ 12/26 (클릭)   │ ─────────►   │ 세션 목록     │ 대화 내용            │
│ 12/25          │              │ ┌───────────┐ │ ┌─────────────────┐ │
│ 12/24          │              │ │ 세션 1    │ │ │ 사용자: 질문... │ │
└────────────────┘              │ │ 세션 2    │ │ │ AI: 답변...     │ │
                                │ └───────────┘ │ └─────────────────┘ │
...
│                       │                                                 │
│  ┌─────────────────┐  │   ┌─────────────────────────────────────────┐  │
│  │ 출장 경비 정산... │  │   │ 연차휴가 신청 절차는 다음과 같습니다:    │  │
│  │ 오후 3:15       │  │   │                                         │  │
│  │ 6개 메시지      │  │   │ 1. 연차 신청서 작성                      │  │
│  │ 응답: 3.1s     │  │   │ 2. 팀장 승인 → 인사팀 확인              │  │
│  └─────────────────┘  │   │ 3. 휴가 사용                            │  │
│                       │   │                                         │  │
│                       │   │ 오후 2:30 | 2.4s | 394 / 377 토큰       │  │
│                       │   │ 참조 문서: 인사규정.pdf                  │  │
│                       │   └─────────────────────────────────────────┘  │
│                       │                                                 │
└───────────────────────┴─────────────────────────────────────────────────┘
...
| 항목 | 설명 |
|------|------|
| **첫 번째 질문** | 사용자의 첫 질문 (100자까지) |
| **시작 시간** | 세션 시작 시각 |
| **메시지 수** | 해당 세션의 총 메시지 개수 |
| **총 응답 시간** | AI 응답에 소요된 총 시간 |
| **토큰 사용량** | 입력/출력 토큰 합계 |

### 대화 내용 상세

세션을 선택하면 우측에 대화 내용이 표시됩니다:

- **사용자 메시지**: 파란색 배경 (우측 정렬)
- **AI 응답**: 회색 배경 (좌측 정렬)
  - 응답 시간 (초 단위)
  - 토큰 사용량 (입력/출력)
  - 참조 문서 목록
...
특정 키워드가 포함된 대화를 찾을 수 있습니다:

1. 검색창에 키워드 입력 (예: "휴가", "출장")
2. **검색** 버튼 클릭
3. 해당 키워드가 포함된 세션만 필터링
4. **초기화** 버튼으로 필터 해제
...
| 메서드 | 경로 | 설명 |
|--------|------|------|
| GET | /api/v1/chatbots/{id}/conversations?date=YYYY-MM-DD | 특정 날짜의 세션 목록 |
| GET | /api/v1/chatbots/{id}/conversations/{session_id} | 세션 상세 메시지 |
